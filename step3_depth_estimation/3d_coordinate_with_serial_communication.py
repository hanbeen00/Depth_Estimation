...
# 시리얼 포트 설정 (보드레이트를 높임)
ser = serial.Serial('/dev/ttyAMA0', 115200, timeout=0.1)  # 보드레이트를 115200으로 설정
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if results_right.multi_hand_landmarks:
            ...
            wrist_right = np.array([int(results_right.multi_hand_landmarks[0].landmark[9].x * w),
        ...
        if results_left.multi_hand_landmarks:
            ...
            wrist_left = np.array([int(results_left.multi_hand_landmarks[0].landmark[9].x * w),
        ...
        if wrist_right is not None and wrist_left is not None:
            avg_x = (wrist_right[0] + wrist_left[0]) // 2
            avg_y = (wrist_right[1] + wrist_left[1]) // 2
            depth = tri.find_depth(wrist_right, wrist_left, frame_right, frame_left, B, f, alpha)

            # 데이터가 변경되었을 경우에만 전송
            data = f"{avg_x},{avg_y},{round(depth, 1)*10}\n"  # 아두이노로 전송할 데이터

            if data != last_data:  # 데이터 변경 확인
                last_data = data
                if ser.is_open:
                    ser.write(data.encode())  # 데이터 전송
                    print(f"Sent: {data}")  # 전송한 데이터 출력
